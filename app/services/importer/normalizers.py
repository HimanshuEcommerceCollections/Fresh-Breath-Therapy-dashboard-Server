...
_ENUM_HINTS: dict[str, tuple[str, ...]] = {
    # ContactStatus. Retired vocabulary (consultation_scheduled,
    # consultation_completed, therapy_session_booked, completed_program,
    # inactive_client) is kept here as SYNONYMS rather than dropped: historical
    # spreadsheets are full of those words, and every hint only ever pre-fills
    # the value-mapping screen for an admin to confirm.
    "new_lead": ("new", "fresh", "enquiry", "inquiry", "open", "unworked"),
    "contacted": ("called", "reached", "phoned", "emailed", "spoke"),
    "follow_up": ("follow up", "followup", "followed up", "chasing",
                  "call back", "callback", "revisit"),
    "awaiting_client_response": ("awaiting client", "waiting on client",
                                 "no response", "awaiting reply",
                                 "pending client", "left message"),
    "awaiting_therapist_insurance_confirmation": (
        "awaiting therapist", "awaiting insurance", "waiting on therapist",
        "waiting on insurance", "pending insurance", "insurance pending",
        "pending confirmation", "awaiting confirmation", "verifying benefits",
    ),
    "booked": ("consult booked", "consultation booked", "scheduled",
               "appointment set", "therapy booked", "session booked",
               "consultation scheduled", "therapy session booked", "starting"),
    "ongoing_therapy": ("ongoing", "in progress", "active", "in therapy",
                        "current", "attending", "in treatment",
                        "consultation completed", "consulted", "intake done"),
    "closed_inactive": ("closed", "inactive", "dropped", "lapsed", "lost",
                        "dormant", "no longer active", "churned", "completed",
                        "done", "finished", "graduated", "discharged",
                        "completed program", "inactive client"),
